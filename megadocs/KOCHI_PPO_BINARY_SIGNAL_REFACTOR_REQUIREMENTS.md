# Kochi Binary-Signal Refactor: BUY/SELL source, Router/Sizer thresholds

**Generated**: 2025-09-08 11:31:00
**Source Directory**: /Users/yeogirlyun/C++/sentio_cpp
**Description**: This document specifies the changes to refactor Kochi PPO from a 3-action actor (HOLD/LONG/SHORT) into a binary signal model producing a single logit per bar (probability of next-bar up after sigmoid). Router/Sizer own HOLD/STRONG variants and position sizing via thresholds and risk controls. Scope includes: freezing Kochi features into a versioned spec; Python trainer producing TorchScript (bce-with-logits), scaler baked-in; C++ inference context returning p_up per bar; updates to strategy glue to emit binary StrategySignal; router/sizer thresholds and cooldown; audit schema to record (p_up, decision); migration/versioning, tests, and replay hooks. Modules indexed: Kochi trainers, feature cache tools, feature providers/projectors, Kochi contexts, router/sizer/signal gate, audit, and CLI.
