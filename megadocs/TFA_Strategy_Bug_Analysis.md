# TFA Strategy Critical Bug Analysis

**Generated**: 2025-09-10 15:47:08
**Source Directory**: /Users/yeogirlyun/C++/sentio_cpp
**Description**: Comprehensive analysis of TFA strategy failure including model architecture, training process, and signal generation issues
...
# TFA Strategy Critical Bug Report

## Executive Summary

The TFA (Transformer Financial Analysis) strategy is experiencing a critical failure where the machine learning model produces only neutral signals (0.5 probability) for all market conditions, resulting in zero trading activity and complete strategy failure.

## Bug Classification

- **Severity**: CRITICAL
- **Priority**: HIGH
- **Category**: Machine Learning Model Failure
- **Impact**: Complete strategy non-functionality

## Problem Description

### Current Behavior
- TFA model generates only neutral signals (probability = 0.5) for all market conditions
- No BUY or SELL signals are produced
- Zero trades executed during backtesting
- Strategy shows 0.00% return with LOW_FREQ health status

### Expected Behavior
- Model should generate varied probability signals (0.0 to 1.0) based on market conditions
- Should produce BUY signals (probability > 0.6) and SELL signals (probability < 0.4)
- Should execute trades based on signal strength and market conditions
- Should show meaningful performance metrics

## Root Cause Analysis

### 1. Model Architecture Issues
- **Simple MLP Architecture**: The current model uses a basic 3-layer MLP (55 → 64 → 64 → 1)
- **Insufficient Complexity**: Financial time series data requires more sophisticated architectures
- **No Temporal Modeling**: MLP cannot capture sequential dependencies in market data

### 2. Training Data Problems
- **Label Quality**: Binary classification (up/down) may be too simplistic
- **Feature Engineering**: 55-feature set may lack predictive signal
- **Data Distribution**: Model may not be learning from diverse market regimes

### 3. Training Process Issues
- **Flat Predictions**: Model produces identical probabilities for all inputs
- **No Learning**: Loss converges but model doesn't differentiate between market conditions
- **Overfitting Prevention**: Model may be too regularized or constrained

### 4. Signal Processing Issues
- **Threshold Logic**: Rolling threshold mechanism may be preventing signal generation
- **Cooldown Periods**: May be too restrictive for signal generation
- **Confidence Floor**: May be filtering out all signals

## Technical Details

### Model Output Analysis
```
[SMOKE] logits mean=0.0180 std=0.0000 | probs mean=0.5045 std=0.0000
[WARNING] Model is flat (std=0.000000), but continuing...
```

### Signal Generation Logs
```
[TFA calc] prob=0.5 thrL=0.6 thrS=0.4 type=HOLD
[TFA calc] prob=0.5 thrL=0.6 thrS=0.4 type=HOLD
[TFA calc] prob=0.5 thrL=0.6 thrS=0.4 type=HOLD
```

### Performance Metrics
- **Total Trades**: 0 (vs. 28 with old model)
- **Return**: 0.00% (vs. -0.14% with old model)
- **Sharpe Ratio**: 0.000
- **Health Status**: LOW_FREQ

## Affected Components

### Core Files
1. `src/strategy_tfa.cpp` - Main strategy implementation
2. `include/sentio/strategy_tfa.hpp` - Strategy header
3. `sentio_trainer/trainers/tfa_fast.py` - Training script
4. `configs/tfa.yaml` - Configuration file
5. `configs/features/feature_spec_55_minimal.json` - Feature specification

### Dependencies
1. `sentio_trainer/models/tfa_transformer.py` - Model architecture
2. `sentio_trainer/utils/feature_cache_hardened.py` - Feature caching
3. `sentio_trainer/utils/schema_meta.py` - Schema validation
4. `artifacts/TFA/v1/` - Model artifacts directory
...
2. **Run Backtest**:
   ```bash
   ./build/sentio_cli tpatest QQQ --strategy TFA --quarters 1
   ```

3. **Observe Results**:
   - Check for neutral signals in logs
   - Verify zero trades in audit
   - Confirm 0.00% return

## Expected vs Actual Output

### Expected
- Varied probability signals (0.1 to 0.9)
- BUY/SELL signal generation
- Meaningful trading activity
- Performance metrics showing strategy effectiveness

### Actual
- All signals neutral (0.5 probability)
- No BUY/SELL signals generated
- Zero trading activity
- 0.00% return with LOW_FREQ status

## Impact Assessment

### Immediate Impact
- TFA strategy completely non-functional
- No trading opportunities identified
- Strategy marked as LOW_FREQ (unhealthy)

### Business Impact
- Loss of potential trading strategy
- Reduced portfolio diversification
- Strategy development time wasted

### Technical Impact
- Model training resources wasted
- Audit system shows no activity
- Performance metrics meaningless

## Proposed Solutions

### Short-term Fixes
1. **Lower Confidence Thresholds**: Reduce `conf_floor` from 0.6 to 0.55
2. **Adjust Signal Logic**: Modify threshold calculation in `calculate_probability`
3. **Disable Cooldown**: Temporarily remove cooldown periods for testing

### Medium-term Solutions
1. **Model Architecture Upgrade**: Implement LSTM or Transformer architecture
2. **Feature Engineering**: Expand feature set with market microstructure data
3. **Label Engineering**: Use magnitude-based or volatility-adjusted labels

### Long-term Solutions
1. **Ensemble Methods**: Combine multiple models for robust predictions
2. **Online Learning**: Implement continuous model updates
3. **Market Regime Detection**: Add regime-aware signal generation

## Testing Strategy

### Unit Tests
- Test model prediction variance
...
- Signal variance (should be > 0.005)
- Trading frequency (should be > 0.1 trades/day)
- Signal distribution (should show BUY/SELL signals)
- Model confidence (should vary across market conditions)

### Alert Conditions
- All signals neutral for > 1000 bars
- Zero trades in > 1 day
- Signal variance < 0.001
- Model confidence flat for extended periods

## Conclusion

The TFA strategy is experiencing a critical failure due to a fundamentally flawed machine learning model that cannot differentiate between market conditions. The model produces only neutral signals, resulting in zero trading activity and complete strategy non-functionality. This requires immediate attention and a comprehensive overhaul of the model architecture, training process, and signal generation logic.

## Next Steps

1. **Immediate**: Implement short-term fixes to restore basic functionality
2. **Short-term**: Redesign model architecture with proper temporal modeling
3. **Medium-term**: Implement comprehensive testing and monitoring
4. **Long-term**: Develop robust ensemble-based approach for production use
...
7. [include/sentio/base_strategy.hpp](#file-7)
8. [include/sentio/binio.hpp](#file-8)
9. [include/sentio/bo.hpp](#file-9)
10. [include/sentio/bollinger.hpp](#file-10)
11. [include/sentio/calendar_seed.hpp](#file-11)
12. [include/sentio/core.hpp](#file-12)
13. [include/sentio/core/bar.hpp](#file-13)
14. [include/sentio/cost_aware_gate.hpp](#file-14)
15. [include/sentio/cost_model.hpp](#file-15)
16. [include/sentio/csv_loader.hpp](#file-16)
17. [include/sentio/data_resolver.hpp](#file-17)
18. [include/sentio/day_index.hpp](#file-18)
19. [include/sentio/exec/asof_index.hpp](#file-19)
20. [include/sentio/exec_types.hpp](#file-20)
21. [include/sentio/execution/pnl_engine.hpp](#file-21)
22. [include/sentio/feature/column_projector.hpp](#file-22)
23. [include/sentio/feature/column_projector_safe.hpp](#file-23)
24. [include/sentio/feature/csv_feature_provider.hpp](#file-24)
25. [include/sentio/feature/feature_builder_guarded.hpp](#file-25)
26. [include/sentio/feature/feature_builder_ops.hpp](#file-26)
27. [include/sentio/feature/feature_feeder_guarded.hpp](#file-27)
28. [include/sentio/feature/feature_from_spec.hpp](#file-28)
29. [include/sentio/feature/feature_matrix.hpp](#file-29)
30. [include/sentio/feature/feature_provider.hpp](#file-30)
31. [include/sentio/feature/name_diff.hpp](#file-31)
32. [include/sentio/feature/ops.hpp](#file-32)
33. [include/sentio/feature/sanitize.hpp](#file-33)
34. [include/sentio/feature/standard_scaler.hpp](#file-34)
35. [include/sentio/feature_builder.hpp](#file-35)
36. [include/sentio/feature_cache.hpp](#file-36)
37. [include/sentio/feature_engineering/feature_normalizer.hpp](#file-37)
38. [include/sentio/feature_engineering/kochi_features.hpp](#file-38)
39. [include/sentio/feature_engineering/technical_indicators.hpp](#file-39)
40. [include/sentio/feature_feeder.hpp](#file-40)
41. [include/sentio/feature_health.hpp](#file-41)
42. [include/sentio/feature_utils.hpp](#file-42)
43. [include/sentio/indicators.hpp](#file-43)
44. [include/sentio/metrics.hpp](#file-44)
45. [include/sentio/ml/feature_pipeline.hpp](#file-45)
46. [include/sentio/ml/feature_window.hpp](#file-46)
47. [include/sentio/ml/iml_model.hpp](#file-47)
48. [include/sentio/ml/model_registry.hpp](#file-48)
49. [include/sentio/ml/ts_model.hpp](#file-49)
50. [include/sentio/of_index.hpp](#file-50)
51. [include/sentio/of_precompute.hpp](#file-51)
52. [include/sentio/optimizer.hpp](#file-52)
53. [include/sentio/orderflow_types.hpp](#file-53)
54. [include/sentio/pnl_accounting.hpp](#file-54)
55. [include/sentio/polygon_client.hpp](#file-55)
56. [include/sentio/polygon_ingest.hpp](#file-56)
57. [include/sentio/portfolio/alpaca_fee_model.hpp](#file-57)
58. [include/sentio/portfolio/capital_manager.hpp](#file-58)
59. [include/sentio/portfolio/fee_model.hpp](#file-59)
60. [include/sentio/portfolio/portfolio_allocator.hpp](#file-60)
61. [include/sentio/portfolio/tc_slippage_model.hpp](#file-61)
62. [include/sentio/portfolio/utilization_governor.hpp](#file-62)
63. [include/sentio/position_manager.hpp](#file-63)
64. [include/sentio/pricebook.hpp](#file-64)
65. [include/sentio/profiling.hpp](#file-65)
66. [include/sentio/progress_bar.hpp](#file-66)
67. [include/sentio/property_test.hpp](#file-67)
68. [include/sentio/rolling_stats.hpp](#file-68)
69. [include/sentio/router.hpp](#file-69)
70. [include/sentio/rth_calendar.hpp](#file-70)
71. [include/sentio/rules/adapters.hpp](#file-71)
72. [include/sentio/rules/bbands_squeeze_rule.hpp](#file-72)
73. [include/sentio/rules/diversity_weighter.hpp](#file-73)
74. [include/sentio/rules/integrated_rule_ensemble.hpp](#file-74)
75. [include/sentio/rules/irule.hpp](#file-75)
76. [include/sentio/rules/momentum_volume_rule.hpp](#file-76)
77. [include/sentio/rules/ofi_proxy_rule.hpp](#file-77)
78. [include/sentio/rules/online_platt_calibrator.hpp](#file-78)
79. [include/sentio/rules/opening_range_breakout_rule.hpp](#file-79)
80. [include/sentio/rules/registry.hpp](#file-80)
81. [include/sentio/rules/sma_cross_rule.hpp](#file-81)
82. [include/sentio/rules/vwap_reversion_rule.hpp](#file-82)
83. [include/sentio/runner.hpp](#file-83)
84. [include/sentio/sanity.hpp](#file-84)
85. [include/sentio/signal_diag.hpp](#file-85)
86. [include/sentio/signal_engine.hpp](#file-86)
87. [include/sentio/signal_gate.hpp](#file-87)
88. [include/sentio/signal_pipeline.hpp](#file-88)
89. [include/sentio/signal_trace.hpp](#file-89)
90. [include/sentio/sim_data.hpp](#file-90)
91. [include/sentio/sizer.hpp](#file-91)
92. [include/sentio/strategy/intraday_position_governor.hpp](#file-92)
93. [include/sentio/strategy_bollinger_squeeze_breakout.hpp](#file-93)
94. [include/sentio/strategy_hybrid_ppo.hpp](#file-94)
95. [include/sentio/strategy_ire.hpp](#file-95)
96. [include/sentio/strategy_kochi_ppo.hpp](#file-96)
97. [include/sentio/strategy_market_making.hpp](#file-97)
98. [include/sentio/strategy_momentum_volume.hpp](#file-98)
99. [include/sentio/strategy_opening_range_breakout.hpp](#file-99)
100. [include/sentio/strategy_order_flow_imbalance.hpp](#file-100)
101. [include/sentio/strategy_order_flow_scalping.hpp](#file-101)
102. [include/sentio/strategy_registry.hpp](#file-102)
103. [include/sentio/strategy_sma_cross.hpp](#file-103)
104. [include/sentio/strategy_tfa.hpp](#file-104)
105. [include/sentio/strategy_transformer.hpp](#file-105)
106. [include/sentio/strategy_transformer_ts.hpp](#file-106)
107. [include/sentio/strategy_utils.hpp](#file-107)
108. [include/sentio/strategy_vwap_reversion.hpp](#file-108)
109. [include/sentio/sym/leverage_registry.hpp](#file-109)
110. [include/sentio/sym/symbol_utils.hpp](#file-110)
111. [include/sentio/symbol_table.hpp](#file-111)
112. [include/sentio/telemetry_logger.hpp](#file-112)
113. [include/sentio/temporal_analysis.hpp](#file-113)
114. [include/sentio/tfa/artifacts_loader.hpp](#file-114)
115. [include/sentio/tfa/artifacts_safe.hpp](#file-115)
116. [include/sentio/tfa/feature_guard.hpp](#file-116)
117. [include/sentio/tfa/input_shim.hpp](#file-117)
118. [include/sentio/tfa/signal_pipeline.hpp](#file-118)
119. [include/sentio/tfa/tfa_seq_context.hpp](#file-119)
120. [include/sentio/time_utils.hpp](#file-120)
121. [include/sentio/torch/safe_from_blob.hpp](#file-121)
122. [include/sentio/util/bytes.hpp](#file-122)
123. [include/sentio/util/safe_matrix.hpp](#file-123)
124. [include/sentio/wf.hpp](#file-124)
125. [sentio_trainer/README.md](#file-125)
126. [sentio_trainer/__init__.py](#file-126)
127. [sentio_trainer/cli.py](#file-127)
128. [sentio_trainer/models/tfa_transformer.py](#file-128)
129. [sentio_trainer/trainers/__init__.py](#file-129)
130. [sentio_trainer/trainers/kochi_ppo.py](#file-130)
131. [sentio_trainer/trainers/tfa_fast.py](#file-131)
132. [sentio_trainer/trainers/tfa_seq.py](#file-132)
133. [sentio_trainer/utils/__init__.py](#file-133)
134. [sentio_trainer/utils/cache_schema_check.py](#file-134)
135. [sentio_trainer/utils/feature_cache.py](#file-135)
136. [sentio_trainer/utils/feature_cache_hardened.py](#file-136)
137. [sentio_trainer/utils/kochi_features.py](#file-137)
138. [sentio_trainer/utils/schema_meta.py](#file-138)
139. [src/audit.cpp](#file-139)
140. [src/base_strategy.cpp](#file-140)
141. [src/cost_aware_gate.cpp](#file-141)
142. [src/csv_loader.cpp](#file-142)
143. [src/feature_builder.cpp](#file-143)
144. [src/feature_cache.cpp](#file-144)
145. [src/feature_engineering/feature_normalizer.cpp](#file-145)
146. [src/feature_engineering/kochi_features.cpp](#file-146)
147. [src/feature_engineering/technical_indicators.cpp](#file-147)
148. [src/feature_feeder.cpp](#file-148)
149. [src/feature_feeder_guarded.cpp](#file-149)
150. [src/feature_health.cpp](#file-150)
151. [src/kochi_runner.cpp](#file-151)
152. [src/main.cpp](#file-152)
153. [src/ml/model_registry_ts.cpp](#file-153)
154. [src/ml/ts_model.cpp](#file-154)
155. [src/optimizer.cpp](#file-155)
156. [src/pnl_accounting.cpp](#file-156)
157. [src/poly_fetch_main.cpp](#file-157)
158. [src/polygon_client.cpp](#file-158)
159. [src/polygon_ingest.cpp](#file-159)
160. [src/router.cpp](#file-160)
161. [src/rth_calendar.cpp](#file-161)
162. [src/runner.cpp](#file-162)
163. [src/sanity.cpp](#file-163)
164. [src/signal_engine.cpp](#file-164)
165. [src/signal_gate.cpp](#file-165)
166. [src/signal_pipeline.cpp](#file-166)
167. [src/signal_trace.cpp](#file-167)
168. [src/sim_data.cpp](#file-168)
169. [src/strategy/run_rule_ensemble.cpp](#file-169)
170. [src/strategy_bollinger_squeeze_breakout.cpp](#file-170)
171. [src/strategy_hybrid_ppo.cpp](#file-171)
172. [src/strategy_ire.cpp](#file-172)
173. [src/strategy_kochi_ppo.cpp](#file-173)
174. [src/strategy_market_making.cpp](#file-174)
175. [src/strategy_momentum_volume.cpp](#file-175)
176. [src/strategy_opening_range_breakout.cpp](#file-176)
177. [src/strategy_order_flow_imbalance.cpp](#file-177)
178. [src/strategy_order_flow_scalping.cpp](#file-178)
179. [src/strategy_registry.cpp](#file-179)
180. [src/strategy_sma_cross.cpp](#file-180)
181. [src/strategy_tfa.cpp](#file-181)
182. [src/strategy_transformer_ts.cpp](#file-182)
183. [src/strategy_vwap_reversion.cpp](#file-183)
184. [src/telemetry_logger.cpp](#file-184)
185. [src/temporal_analysis.cpp](#file-185)
186. [src/time_utils.cpp](#file-186)

---

## 📄 **FILE 1 of 186**: include/kochi/kochi_binary_context.hpp
...
## 📄 **FILE 2 of 186**: include/kochi/kochi_ppo_context.hpp
...
## 📄 **FILE 3 of 186**: include/sentio/all_strategies.hpp
...
## 📄 **FILE 4 of 186**: include/sentio/alpha.hpp
...
## 📄 **FILE 5 of 186**: include/sentio/alpha/sota_linear_policy.hpp
...
## 📄 **FILE 6 of 186**: include/sentio/audit.hpp
...
## 📄 **FILE 7 of 186**: include/sentio/base_strategy.hpp

**File Information**:
- **Path**: `include/sentio/base_strategy.hpp`

- **Size**: 161 lines
- **Modified**: 2025-09-10 15:15:57
...
## 📄 **FILE 8 of 186**: include/sentio/binio.hpp
...
## 📄 **FILE 9 of 186**: include/sentio/bo.hpp
...
## 📄 **FILE 10 of 186**: include/sentio/bollinger.hpp
...
## 📄 **FILE 11 of 186**: include/sentio/calendar_seed.hpp
...
## 📄 **FILE 12 of 186**: include/sentio/core.hpp
...
## 📄 **FILE 13 of 186**: include/sentio/core/bar.hpp
...
## 📄 **FILE 14 of 186**: include/sentio/cost_aware_gate.hpp
...
## 📄 **FILE 15 of 186**: include/sentio/cost_model.hpp
...
## 📄 **FILE 16 of 186**: include/sentio/csv_loader.hpp
...
## 📄 **FILE 17 of 186**: include/sentio/data_resolver.hpp
...
## 📄 **FILE 18 of 186**: include/sentio/day_index.hpp
...
## 📄 **FILE 19 of 186**: include/sentio/exec/asof_index.hpp
...
## 📄 **FILE 20 of 186**: include/sentio/exec_types.hpp
...
## 📄 **FILE 21 of 186**: include/sentio/execution/pnl_engine.hpp
...
## 📄 **FILE 22 of 186**: include/sentio/feature/column_projector.hpp
...
## 📄 **FILE 23 of 186**: include/sentio/feature/column_projector_safe.hpp
...
## 📄 **FILE 24 of 186**: include/sentio/feature/csv_feature_provider.hpp
...
## 📄 **FILE 25 of 186**: include/sentio/feature/feature_builder_guarded.hpp
...
## 📄 **FILE 26 of 186**: include/sentio/feature/feature_builder_ops.hpp
...
## 📄 **FILE 27 of 186**: include/sentio/feature/feature_feeder_guarded.hpp
...
## 📄 **FILE 28 of 186**: include/sentio/feature/feature_from_spec.hpp
...
## 📄 **FILE 29 of 186**: include/sentio/feature/feature_matrix.hpp
...
## 📄 **FILE 30 of 186**: include/sentio/feature/feature_provider.hpp
...
## 📄 **FILE 31 of 186**: include/sentio/feature/name_diff.hpp
...
## 📄 **FILE 32 of 186**: include/sentio/feature/ops.hpp
...
## 📄 **FILE 33 of 186**: include/sentio/feature/sanitize.hpp
...
## 📄 **FILE 34 of 186**: include/sentio/feature/standard_scaler.hpp
...
## 📄 **FILE 35 of 186**: include/sentio/feature_builder.hpp
...
## 📄 **FILE 36 of 186**: include/sentio/feature_cache.hpp
...
## 📄 **FILE 37 of 186**: include/sentio/feature_engineering/feature_normalizer.hpp
...
## 📄 **FILE 38 of 186**: include/sentio/feature_engineering/kochi_features.hpp
...
## 📄 **FILE 39 of 186**: include/sentio/feature_engineering/technical_indicators.hpp
...
## 📄 **FILE 40 of 186**: include/sentio/feature_feeder.hpp
...
## 📄 **FILE 41 of 186**: include/sentio/feature_health.hpp
...
## 📄 **FILE 42 of 186**: include/sentio/feature_utils.hpp
...
## 📄 **FILE 43 of 186**: include/sentio/indicators.hpp
...
## 📄 **FILE 44 of 186**: include/sentio/metrics.hpp
...
## 📄 **FILE 45 of 186**: include/sentio/ml/feature_pipeline.hpp
...
## 📄 **FILE 46 of 186**: include/sentio/ml/feature_window.hpp
...
## 📄 **FILE 47 of 186**: include/sentio/ml/iml_model.hpp
...
## 📄 **FILE 48 of 186**: include/sentio/ml/model_registry.hpp
...
## 📄 **FILE 49 of 186**: include/sentio/ml/ts_model.hpp
...
## 📄 **FILE 50 of 186**: include/sentio/of_index.hpp
...
## 📄 **FILE 51 of 186**: include/sentio/of_precompute.hpp
...
## 📄 **FILE 52 of 186**: include/sentio/optimizer.hpp
...
## 📄 **FILE 53 of 186**: include/sentio/orderflow_types.hpp
...
## 📄 **FILE 54 of 186**: include/sentio/pnl_accounting.hpp
...
## 📄 **FILE 55 of 186**: include/sentio/polygon_client.hpp
...
## 📄 **FILE 56 of 186**: include/sentio/polygon_ingest.hpp
...
## 📄 **FILE 57 of 186**: include/sentio/portfolio/alpaca_fee_model.hpp
...
## 📄 **FILE 58 of 186**: include/sentio/portfolio/capital_manager.hpp
...
## 📄 **FILE 59 of 186**: include/sentio/portfolio/fee_model.hpp
...
## 📄 **FILE 60 of 186**: include/sentio/portfolio/portfolio_allocator.hpp
...
## 📄 **FILE 61 of 186**: include/sentio/portfolio/tc_slippage_model.hpp
...
## 📄 **FILE 62 of 186**: include/sentio/portfolio/utilization_governor.hpp
...
## 📄 **FILE 63 of 186**: include/sentio/position_manager.hpp
...
## 📄 **FILE 64 of 186**: include/sentio/pricebook.hpp
...
## 📄 **FILE 65 of 186**: include/sentio/profiling.hpp
...
## 📄 **FILE 66 of 186**: include/sentio/progress_bar.hpp
...
## 📄 **FILE 67 of 186**: include/sentio/property_test.hpp
...
## 📄 **FILE 68 of 186**: include/sentio/rolling_stats.hpp
...
## 📄 **FILE 69 of 186**: include/sentio/router.hpp

**File Information**:
- **Path**: `include/sentio/router.hpp`

- **Size**: 92 lines
- **Modified**: 2025-09-10 14:57:31
...
## 📄 **FILE 70 of 186**: include/sentio/rth_calendar.hpp
...
## 📄 **FILE 71 of 186**: include/sentio/rules/adapters.hpp
...
## 📄 **FILE 72 of 186**: include/sentio/rules/bbands_squeeze_rule.hpp
...
## 📄 **FILE 73 of 186**: include/sentio/rules/diversity_weighter.hpp
...
## 📄 **FILE 74 of 186**: include/sentio/rules/integrated_rule_ensemble.hpp
...
## 📄 **FILE 75 of 186**: include/sentio/rules/irule.hpp
...
## 📄 **FILE 76 of 186**: include/sentio/rules/momentum_volume_rule.hpp
...
## 📄 **FILE 77 of 186**: include/sentio/rules/ofi_proxy_rule.hpp
...
## 📄 **FILE 78 of 186**: include/sentio/rules/online_platt_calibrator.hpp
...
## 📄 **FILE 79 of 186**: include/sentio/rules/opening_range_breakout_rule.hpp
...
## 📄 **FILE 80 of 186**: include/sentio/rules/registry.hpp
...
## 📄 **FILE 81 of 186**: include/sentio/rules/sma_cross_rule.hpp
...
## 📄 **FILE 82 of 186**: include/sentio/rules/vwap_reversion_rule.hpp
...
## 📄 **FILE 83 of 186**: include/sentio/runner.hpp
...
## 📄 **FILE 84 of 186**: include/sentio/sanity.hpp
...
## 📄 **FILE 85 of 186**: include/sentio/signal_diag.hpp
...
## 📄 **FILE 86 of 186**: include/sentio/signal_engine.hpp
...
## 📄 **FILE 87 of 186**: include/sentio/signal_gate.hpp
...
## 📄 **FILE 88 of 186**: include/sentio/signal_pipeline.hpp
...
## 📄 **FILE 89 of 186**: include/sentio/signal_trace.hpp
...
## 📄 **FILE 90 of 186**: include/sentio/sim_data.hpp
...
## 📄 **FILE 91 of 186**: include/sentio/sizer.hpp
...
## 📄 **FILE 92 of 186**: include/sentio/strategy/intraday_position_governor.hpp
...
## 📄 **FILE 93 of 186**: include/sentio/strategy_bollinger_squeeze_breakout.hpp
...
## 📄 **FILE 94 of 186**: include/sentio/strategy_hybrid_ppo.hpp
...
## 📄 **FILE 95 of 186**: include/sentio/strategy_ire.hpp
...
## 📄 **FILE 96 of 186**: include/sentio/strategy_kochi_ppo.hpp
...
## 📄 **FILE 97 of 186**: include/sentio/strategy_market_making.hpp
...
## 📄 **FILE 98 of 186**: include/sentio/strategy_momentum_volume.hpp
...
## 📄 **FILE 99 of 186**: include/sentio/strategy_opening_range_breakout.hpp
...
## 📄 **FILE 100 of 186**: include/sentio/strategy_order_flow_imbalance.hpp
...
## 📄 **FILE 101 of 186**: include/sentio/strategy_order_flow_scalping.hpp
...
## 📄 **FILE 102 of 186**: include/sentio/strategy_registry.hpp

**File Information**:
- **Path**: `include/sentio/strategy_registry.hpp`
...
## 📄 **FILE 103 of 186**: include/sentio/strategy_sma_cross.hpp
...
## 📄 **FILE 104 of 186**: include/sentio/strategy_tfa.hpp
...
## 📄 **FILE 105 of 186**: include/sentio/strategy_transformer.hpp
...
## 📄 **FILE 106 of 186**: include/sentio/strategy_transformer_ts.hpp
...
## 📄 **FILE 107 of 186**: include/sentio/strategy_utils.hpp
...
## 📄 **FILE 108 of 186**: include/sentio/strategy_vwap_reversion.hpp
...
## 📄 **FILE 109 of 186**: include/sentio/sym/leverage_registry.hpp
...
## 📄 **FILE 110 of 186**: include/sentio/sym/symbol_utils.hpp
...
## 📄 **FILE 111 of 186**: include/sentio/symbol_table.hpp
...
## 📄 **FILE 112 of 186**: include/sentio/telemetry_logger.hpp
...
## 📄 **FILE 113 of 186**: include/sentio/temporal_analysis.hpp

**File Information**:
- **Path**: `include/sentio/temporal_analysis.hpp`

- **Size**: 397 lines
- **Modified**: 2025-09-10 13:17:35
...
## 📄 **FILE 114 of 186**: include/sentio/tfa/artifacts_loader.hpp
...
## 📄 **FILE 115 of 186**: include/sentio/tfa/artifacts_safe.hpp
...
## 📄 **FILE 116 of 186**: include/sentio/tfa/feature_guard.hpp
...
## 📄 **FILE 117 of 186**: include/sentio/tfa/input_shim.hpp
...
## 📄 **FILE 118 of 186**: include/sentio/tfa/signal_pipeline.hpp
...
## 📄 **FILE 119 of 186**: include/sentio/tfa/tfa_seq_context.hpp
...
## 📄 **FILE 120 of 186**: include/sentio/time_utils.hpp
...
## 📄 **FILE 121 of 186**: include/sentio/torch/safe_from_blob.hpp
...
## 📄 **FILE 122 of 186**: include/sentio/util/bytes.hpp
...
## 📄 **FILE 123 of 186**: include/sentio/util/safe_matrix.hpp
...
## 📄 **FILE 124 of 186**: include/sentio/wf.hpp
...
## 📄 **FILE 125 of 186**: sentio_trainer/README.md
...
## 📄 **FILE 126 of 186**: sentio_trainer/__init__.py
...
## 📄 **FILE 127 of 186**: sentio_trainer/cli.py
...
## 📄 **FILE 128 of 186**: sentio_trainer/models/tfa_transformer.py
...
## 📄 **FILE 129 of 186**: sentio_trainer/trainers/__init__.py
...
## 📄 **FILE 130 of 186**: sentio_trainer/trainers/kochi_ppo.py
...
## 📄 **FILE 131 of 186**: sentio_trainer/trainers/tfa_fast.py

**File Information**:
- **Path**: `sentio_trainer/trainers/tfa_fast.py`

- **Size**: 299 lines
- **Modified**: 2025-09-10 15:44:22
...
def train_tfa_fast(
    ...
    with torch.inference_mode():
        ...
        print(f"[WARNING] Model is flat (std={probs.std():.6f}), but continuing...")
...
## 📄 **FILE 132 of 186**: sentio_trainer/trainers/tfa_seq.py
...
## 📄 **FILE 133 of 186**: sentio_trainer/utils/__init__.py
...
## 📄 **FILE 134 of 186**: sentio_trainer/utils/cache_schema_check.py
...
## 📄 **FILE 135 of 186**: sentio_trainer/utils/feature_cache.py
...
## 📄 **FILE 136 of 186**: sentio_trainer/utils/feature_cache_hardened.py
...
## 📄 **FILE 137 of 186**: sentio_trainer/utils/kochi_features.py
...
## 📄 **FILE 138 of 186**: sentio_trainer/utils/schema_meta.py
...
## 📄 **FILE 140 of 186**: src/base_strategy.cpp
...
## 📄 **FILE 141 of 186**: src/cost_aware_gate.cpp
...
## 📄 **FILE 142 of 186**: src/csv_loader.cpp
...
## 📄 **FILE 143 of 186**: src/feature_builder.cpp
...
## 📄 **FILE 144 of 186**: src/feature_cache.cpp
...
## 📄 **FILE 145 of 186**: src/feature_engineering/feature_normalizer.cpp
...
## 📄 **FILE 146 of 186**: src/feature_engineering/kochi_features.cpp
...
## 📄 **FILE 147 of 186**: src/feature_engineering/technical_indicators.cpp
...
## 📄 **FILE 148 of 186**: src/feature_feeder.cpp
...
## 📄 **FILE 149 of 186**: src/feature_feeder_guarded.cpp
...
## 📄 **FILE 150 of 186**: src/feature_health.cpp
...
## 📄 **FILE 151 of 186**: src/kochi_runner.cpp
...
## 📄 **FILE 153 of 186**: src/ml/model_registry_ts.cpp
...
## 📄 **FILE 154 of 186**: src/ml/ts_model.cpp
...
## 📄 **FILE 155 of 186**: src/optimizer.cpp
...
## 📄 **FILE 156 of 186**: src/pnl_accounting.cpp
...
## 📄 **FILE 157 of 186**: src/poly_fetch_main.cpp
...
## 📄 **FILE 158 of 186**: src/polygon_client.cpp
...
## 📄 **FILE 159 of 186**: src/polygon_ingest.cpp
...
## 📄 **FILE 160 of 186**: src/router.cpp
...
## 📄 **FILE 161 of 186**: src/rth_calendar.cpp
...
## 📄 **FILE 162 of 186**: src/runner.cpp
...
## 📄 **FILE 163 of 186**: src/sanity.cpp
...
## 📄 **FILE 164 of 186**: src/signal_engine.cpp
...
## 📄 **FILE 165 of 186**: src/signal_gate.cpp
...
## 📄 **FILE 166 of 186**: src/signal_pipeline.cpp
...
## 📄 **FILE 167 of 186**: src/signal_trace.cpp
...
## 📄 **FILE 168 of 186**: src/sim_data.cpp
...
## 📄 **FILE 169 of 186**: src/strategy/run_rule_ensemble.cpp
...
## 📄 **FILE 170 of 186**: src/strategy_bollinger_squeeze_breakout.cpp
...
## 📄 **FILE 171 of 186**: src/strategy_hybrid_ppo.cpp
...
## 📄 **FILE 172 of 186**: src/strategy_ire.cpp
...
## 📄 **FILE 173 of 186**: src/strategy_kochi_ppo.cpp
...
## 📄 **FILE 174 of 186**: src/strategy_market_making.cpp
...
## 📄 **FILE 175 of 186**: src/strategy_momentum_volume.cpp
...
## 📄 **FILE 176 of 186**: src/strategy_opening_range_breakout.cpp
...
## 📄 **FILE 177 of 186**: src/strategy_order_flow_imbalance.cpp
...
## 📄 **FILE 178 of 186**: src/strategy_order_flow_scalping.cpp
...
## 📄 **FILE 179 of 186**: src/strategy_registry.cpp

**File Information**:
- **Path**: `src/strategy_registry.cpp`
...
## 📄 **FILE 180 of 186**: src/strategy_sma_cross.cpp
...
## 📄 **FILE 181 of 186**: src/strategy_tfa.cpp
...
## 📄 **FILE 182 of 186**: src/strategy_transformer_ts.cpp
...
## 📄 **FILE 183 of 186**: src/strategy_vwap_reversion.cpp
...
## 📄 **FILE 184 of 186**: src/telemetry_logger.cpp
...
## 📄 **FILE 185 of 186**: src/temporal_analysis.cpp

**File Information**:
- **Path**: `src/temporal_analysis.cpp`

- **Size**: 173 lines
- **Modified**: 2025-09-10 13:04:05
...
## 📄 **FILE 186 of 186**: src/time_utils.cpp
