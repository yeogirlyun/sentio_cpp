# TPA Metrics Evaluation Inconsistency Analysis

**Generated**: 2025-09-13 00:07:25
**Source Directory**: /Users/yeogirlyun/C++/sentio_cpp
**Description**: Comprehensive analysis of TPA vs Audit metrics calculation discrepancies and unified solution approach
...
# TPA Metrics Evaluation Inconsistency Bug Report

## Executive Summary

**Critical Issue**: The Temporal Performance Analysis (TPA) system and Audit system produce significantly different Monthly Projected Return (MPR) calculations for identical trading data, creating inconsistent strategy evaluation metrics that could lead to poor live trading performance.

**Impact**: Strategy evaluation inconsistency prevents accurate assessment of trading performance, potentially leading to deployment of suboptimal strategies in live trading environments.

**Severity**: HIGH - Affects core strategy evaluation and live trading decisions

## Problem Description

### Observed Behavior
- **TPA Test Results**: Monthly Return: 20.27%, Sharpe Ratio: 10.287
- **Audit Results**: Monthly Projected Return (MPR): 0.59%, Sharpe Ratio: 4.528
- **Trade Count**: Both systems correctly show 806 trades (data consistency confirmed)

### Root Cause Analysis

#### 1. **Different Calculation Methodologies**
The systems use fundamentally different approaches to calculate performance metrics:

**TPA System (compute_metrics_day_aware)**:
- Compresses minute-level equity curve to daily closes
- Uses proper compound interest: `std::pow(1.0 + annual_return, 1.0/12.0) - 1.0`
- Calculates Sharpe ratio from daily return variance
- More sophisticated statistical approach

**Audit System (audit_db.cpp)**:
- Uses raw P&L sum divided by time period
- Previously used simple division (FIXED), now uses compound interest
- Simplified Sharpe calculation based on P&L volatility
- Direct P&L-based approach

#### 2. **Data Processing Differences**
- **TPA**: Processes full equity curve with day-aware compression
- **Audit**: Processes discrete fill events and P&L deltas
- **Time Period**: Both use same data range but different granularity

#### 3. **Mathematical Inconsistencies**
The core issue is that both systems claim to calculate "monthly projected return" but use different base data:
- **TPA**: Uses equity curve progression (mark-to-market)
- **Audit**: Uses realized P&L only

## Technical Details

### Affected Components
1. **src/temporal_analysis.cpp** - TPA calculation logic
2. **include/sentio/metrics.hpp** - Core metrics computation
3. **src/runner.cpp** - Backtest execution and metrics population
4. **audit/src/audit_db.cpp** - Audit system metrics calculation
5. **audit/src/audit_cli.cpp** - Audit reporting interface

### Key Code Sections

**TPA Calculation (CORRECT)**:
```cpp
// From include/sentio/metrics.hpp:103
s.monthly_proj = std::pow(1.0 + s.ret_ann, 1.0/12.0) - 1.0;
```

**Audit Calculation (RECENTLY FIXED)**:
```cpp
// From audit/src/audit_db.cpp:224-229
double total_return_decimal = s.total_return / 100.0;
double years = time_span_days / 365.25;
double annual_return = years > 0 ? std::pow(1.0 + total_return_decimal, 1.0/years) - 1.0 : 0.0;
...
### Current Problems
1. **Strategy Evaluation Uncertainty**: Cannot reliably assess which strategies are truly profitable
2. **Live Trading Risk**: Deploying strategies based on inconsistent metrics could lead to losses
3. **Performance Tracking**: Unable to accurately track strategy performance over time
4. **Optimization Challenges**: Cannot optimize strategies effectively with inconsistent feedback

### Financial Risk
- **False Positives**: Strategies showing 20% monthly returns may actually deliver 0.6%
- **Capital Allocation**: Incorrect metrics lead to suboptimal capital deployment
- **Risk Management**: Inconsistent Sharpe ratios affect risk assessment

## Proposed Solution

### 1. **Standardize on Single Metrics Engine**
**Recommendation**: Use `compute_metrics_day_aware` as the single source of truth for all performance calculations.

**Rationale**:
- More statistically robust (day-aware compression)
- Handles equity curve progression properly
- Consistent with industry standards
- Already battle-tested in TPA system

### 2. **Implementation Strategy**

#### Phase 1: Audit System Alignment
- **Modify audit/src/audit_db.cpp** to use `compute_metrics_day_aware`
- **Reconstruct equity curve** from audit events for proper calculation
- **Maintain backward compatibility** with existing audit database

#### Phase 2: Validation Framework
- **Cross-validation tests** between TPA and Audit results
- **Automated regression testing** for metrics consistency
- **Performance benchmarking** against known reference calculations

#### Phase 3: Live Trading Integration
- **Real-time metrics calculation** using standardized engine
- **Performance monitoring** with consistent metrics
- **Strategy evaluation pipeline** using unified calculations

### 3. **Technical Implementation**

#### Create Unified Metrics Service
```cpp
// New: include/sentio/unified_metrics.hpp
...
#### Modify Audit System
```cpp
// Modified: audit/src/audit_db.cpp
DB::Summary DB::summarize(const std::string& run_id) {
    // 1. Reconstruct equity curve from events
    auto equity_curve = reconstruct_equity_curve(run_id);
    
    // 2. Use unified metrics calculator
    auto unified_summary = UnifiedMetricsCalculator::calculate_from_equity_curve(
        equity_curve, n_fill
    );
    
    // 3. Convert to audit Summary format
    return convert_to_audit_summary(unified_summary);
}
```
...
#### Validation Criteria
- **Metrics Consistency**: TPA and Audit must show <1% difference in MPR
- **Mathematical Accuracy**: All calculations must use proper compound interest
- **Data Integrity**: Trade counts and P&L sums must match exactly
- **Performance**: Unified calculation must not significantly impact performance

#### Testing Strategy
- **Unit Tests**: Test unified metrics calculator with known datasets
- **Integration Tests**: Validate TPA and Audit consistency
- **Regression Tests**: Ensure existing functionality remains intact
- **Performance Tests**: Benchmark calculation speed and memory usage

## Expected Outcomes

### Short Term (1-2 weeks)
- **Consistent Metrics**: TPA and Audit show identical MPR calculations
- **Reliable Evaluation**: Strategy assessment becomes trustworthy
- **Reduced Confusion**: Single source of truth for performance metrics

### Medium Term (1-2 months)
- **Improved Strategy Selection**: Better strategies identified for live trading
- **Enhanced Risk Management**: Accurate Sharpe ratios improve risk assessment
- **Optimized Capital Allocation**: Correct MPR enables better position sizing

### Long Term (3-6 months)
- **Higher Live Trading Returns**: Accurate evaluation leads to better strategy deployment
- **Reduced Drawdowns**: Better risk metrics prevent over-leveraging
- **Scalable Evaluation**: Unified system supports larger strategy portfolios

## Risk Assessment

### Implementation Risks
- **Breaking Changes**: Modifying audit system could affect existing workflows
- **Performance Impact**: Unified calculation might be slower than current audit method
- **Data Migration**: Existing audit data might need recalculation

### Mitigation Strategies
- **Phased Rollout**: Implement alongside existing system initially
...
- **Performance Optimization**: Profile and optimize unified calculator

## Success Metrics

### Technical Metrics
- **Consistency**: <1% difference between TPA and Audit MPR calculations
- **Accuracy**: Metrics match manual calculations within 0.1%
- **Performance**: Calculation time <2x current audit system
- **Coverage**: 100% of existing audit functionality preserved

### Business Metrics
- **Strategy Performance**: Improved correlation between backtest and live results
- **Risk-Adjusted Returns**: Better Sharpe ratios in live trading
- **Capital Efficiency**: Improved return per unit of risk
- **Operational Efficiency**: Reduced time spent reconciling different metrics

## Conclusion

The TPA metrics evaluation inconsistency represents a critical flaw in the strategy evaluation pipeline that must be addressed before deploying strategies in live trading. The proposed unified metrics approach will ensure consistent, accurate performance evaluation, leading to better strategy selection and improved live trading results.

**Priority**: CRITICAL - Must be resolved before any live trading deployment
**Timeline**: 2-4 weeks for complete implementation and validation
**Resources**: 1 senior developer, extensive testing infrastructure
...
1. [audit/include/audit/audit_cli.hpp](#file-1)
2. [audit/include/audit/audit_db.hpp](#file-2)
3. [audit/include/audit/audit_db_recorder.hpp](#file-3)
4. [audit/include/audit/clock.hpp](#file-4)
5. [audit/include/audit/hash.hpp](#file-5)
6. [audit/include/audit/price_csv.hpp](#file-6)
7. [audit/src/audit_cli.cpp](#file-7)
8. [audit/src/audit_db.cpp](#file-8)
9. [audit/src/audit_db_recorder.cpp](#file-9)
10. [audit/src/clock.cpp](#file-10)
11. [audit/src/hash.cpp](#file-11)
12. [audit/src/price_csv.cpp](#file-12)
13. [include/sentio/all_strategies.hpp](#file-13)
14. [include/sentio/alpha.hpp](#file-14)
15. [include/sentio/alpha/sota_linear_policy.hpp](#file-15)
16. [include/sentio/audit.hpp](#file-16)
17. [include/sentio/audit_interface.hpp](#file-17)
18. [include/sentio/audit_validator.hpp](#file-18)
19. [include/sentio/base_strategy.hpp](#file-19)
20. [include/sentio/binio.hpp](#file-20)
21. [include/sentio/bo.hpp](#file-21)
22. [include/sentio/bollinger.hpp](#file-22)
23. [include/sentio/core.hpp](#file-23)
24. [include/sentio/core/bar.hpp](#file-24)
25. [include/sentio/cost_model.hpp](#file-25)
26. [include/sentio/csv_loader.hpp](#file-26)
27. [include/sentio/data_downloader.hpp](#file-27)
28. [include/sentio/data_resolver.hpp](#file-28)
29. [include/sentio/day_index.hpp](#file-29)
...
53. [include/sentio/feature_health.hpp](#file-53)
54. [include/sentio/feature_utils.hpp](#file-54)
55. [include/sentio/indicators.hpp](#file-55)
56. [include/sentio/mars_data_loader.hpp](#file-56)
57. [include/sentio/metrics.hpp](#file-57)
58. [include/sentio/ml/feature_pipeline.hpp](#file-58)
59. [include/sentio/ml/feature_window.hpp](#file-59)
60. [include/sentio/ml/iml_model.hpp](#file-60)
61. [include/sentio/ml/model_registry.hpp](#file-61)
62. [include/sentio/ml/ts_model.hpp](#file-62)
63. [include/sentio/of_index.hpp](#file-63)
64. [include/sentio/of_precompute.hpp](#file-64)
65. [include/sentio/optimizer.hpp](#file-65)
66. [include/sentio/orderflow_types.hpp](#file-66)
67. [include/sentio/pnl_accounting.hpp](#file-67)
68. [include/sentio/polygon_client.hpp](#file-68)
69. [include/sentio/portfolio/alpaca_fee_model.hpp](#file-69)
70. [include/sentio/portfolio/capital_manager.hpp](#file-70)
71. [include/sentio/portfolio/fee_model.hpp](#file-71)
72. [include/sentio/portfolio/portfolio_allocator.hpp](#file-72)
73. [include/sentio/portfolio/tc_slippage_model.hpp](#file-73)
74. [include/sentio/portfolio/utilization_governor.hpp](#file-74)
75. [include/sentio/position_coordinator.hpp](#file-75)
76. [include/sentio/position_guardian.hpp](#file-76)
77. [include/sentio/position_manager.hpp](#file-77)
78. [include/sentio/position_orchestrator.hpp](#file-78)
79. [include/sentio/position_validator.hpp](#file-79)
80. [include/sentio/pricebook.hpp](#file-80)
81. [include/sentio/profiling.hpp](#file-81)
82. [include/sentio/progress_bar.hpp](#file-82)
83. [include/sentio/property_test.hpp](#file-83)
84. [include/sentio/rolling_stats.hpp](#file-84)
85. [include/sentio/router.hpp](#file-85)
86. [include/sentio/rsi_prob.hpp](#file-86)
87. [include/sentio/rsi_strategy.hpp](#file-87)
88. [include/sentio/rules/adapters.hpp](#file-88)
89. [include/sentio/rules/bbands_squeeze_rule.hpp](#file-89)
90. [include/sentio/rules/diversity_weighter.hpp](#file-90)
91. [include/sentio/rules/integrated_rule_ensemble.hpp](#file-91)
92. [include/sentio/rules/irule.hpp](#file-92)
93. [include/sentio/rules/momentum_volume_rule.hpp](#file-93)
94. [include/sentio/rules/ofi_proxy_rule.hpp](#file-94)
95. [include/sentio/rules/online_platt_calibrator.hpp](#file-95)
96. [include/sentio/rules/opening_range_breakout_rule.hpp](#file-96)
97. [include/sentio/rules/registry.hpp](#file-97)
98. [include/sentio/rules/sma_cross_rule.hpp](#file-98)
99. [include/sentio/rules/utils/validation.hpp](#file-99)
100. [include/sentio/rules/vwap_reversion_rule.hpp](#file-100)
101. [include/sentio/runner.hpp](#file-101)
102. [include/sentio/sanity.hpp](#file-102)
103. [include/sentio/side.hpp](#file-103)
104. [include/sentio/signal.hpp](#file-104)
105. [include/sentio/signal_diag.hpp](#file-105)
106. [include/sentio/signal_engine.hpp](#file-106)
107. [include/sentio/signal_gate.hpp](#file-107)
108. [include/sentio/signal_or.hpp](#file-108)
109. [include/sentio/signal_pipeline.hpp](#file-109)
110. [include/sentio/signal_trace.hpp](#file-110)
111. [include/sentio/signal_utils.hpp](#file-111)
112. [include/sentio/sim_data.hpp](#file-112)
113. [include/sentio/sizer.hpp](#file-113)
114. [include/sentio/strategy/intraday_position_governor.hpp](#file-114)
115. [include/sentio/strategy_bollinger_squeeze_breakout.hpp](#file-115)
116. [include/sentio/strategy_ire.hpp](#file-116)
117. [include/sentio/strategy_kochi_ppo.hpp](#file-117)
118. [include/sentio/strategy_market_making.hpp](#file-118)
119. [include/sentio/strategy_momentum_volume.hpp](#file-119)
120. [include/sentio/strategy_opening_range_breakout.hpp](#file-120)
121. [include/sentio/strategy_order_flow_imbalance.hpp](#file-121)
122. [include/sentio/strategy_order_flow_scalping.hpp](#file-122)
123. [include/sentio/strategy_signal_or.hpp](#file-123)
124. [include/sentio/strategy_sma_cross.hpp](#file-124)
125. [include/sentio/strategy_tfa.hpp](#file-125)
126. [include/sentio/strategy_transformer.hpp](#file-126)
127. [include/sentio/strategy_utils.hpp](#file-127)
128. [include/sentio/strategy_vwap_reversion.hpp](#file-128)
129. [include/sentio/sym/leverage_registry.hpp](#file-129)
130. [include/sentio/sym/symbol_utils.hpp](#file-130)
131. [include/sentio/symbol_table.hpp](#file-131)
132. [include/sentio/telemetry_logger.hpp](#file-132)
133. [include/sentio/temporal_analysis.hpp](#file-133)
134. [include/sentio/tfa/artifacts_loader.hpp](#file-134)
135. [include/sentio/tfa/artifacts_safe.hpp](#file-135)
136. [include/sentio/tfa/feature_guard.hpp](#file-136)
137. [include/sentio/tfa/input_shim.hpp](#file-137)
138. [include/sentio/tfa/signal_pipeline.hpp](#file-138)
139. [include/sentio/tfa/tfa_seq_context.hpp](#file-139)
140. [include/sentio/time_utils.hpp](#file-140)
141. [include/sentio/torch/safe_from_blob.hpp](#file-141)
142. [include/sentio/util/bytes.hpp](#file-142)
143. [include/sentio/util/safe_matrix.hpp](#file-143)
144. [include/sentio/utils/formatting.hpp](#file-144)
145. [include/sentio/utils/validation.hpp](#file-145)
146. [include/sentio/virtual_market.hpp](#file-146)
147. [include/sentio/wf.hpp](#file-147)
148. [src/audit.cpp](#file-148)
149. [src/audit_validator.cpp](#file-149)
150. [src/base_strategy.cpp](#file-150)
151. [src/csv_loader.cpp](#file-151)
152. [src/data_downloader.cpp](#file-152)
153. [src/feature_builder.cpp](#file-153)
154. [src/feature_cache.cpp](#file-154)
155. [src/feature_engineering/feature_normalizer.cpp](#file-155)
156. [src/feature_engineering/kochi_features.cpp](#file-156)
157. [src/feature_engineering/technical_indicators.cpp](#file-157)
158. [src/feature_feeder.cpp](#file-158)
159. [src/feature_feeder_guarded.cpp](#file-159)
160. [src/feature_health.cpp](#file-160)
161. [src/main.cpp](#file-161)
162. [src/mars_data_loader.cpp](#file-162)
163. [src/ml/model_registry_ts.cpp](#file-163)
164. [src/ml/ts_model.cpp](#file-164)
165. [src/optimizer.cpp](#file-165)
166. [src/pnl_accounting.cpp](#file-166)
167. [src/poly_fetch_main.cpp](#file-167)
168. [src/polygon_client.cpp](#file-168)
169. [src/position_coordinator.cpp](#file-169)
170. [src/position_guardian.cpp](#file-170)
171. [src/position_orchestrator.cpp](#file-171)
172. [src/router.cpp](#file-172)
173. [src/rsi_strategy.cpp](#file-173)
174. [src/runner.cpp](#file-174)
175. [src/sanity.cpp](#file-175)
176. [src/signal_engine.cpp](#file-176)
177. [src/signal_gate.cpp](#file-177)
178. [src/signal_pipeline.cpp](#file-178)
179. [src/signal_trace.cpp](#file-179)
180. [src/sim_data.cpp](#file-180)
181. [src/strategy/run_rule_ensemble.cpp](#file-181)
182. [src/strategy_bollinger_squeeze_breakout.cpp](#file-182)
183. [src/strategy_initialization.cpp](#file-183)
184. [src/strategy_ire.cpp](#file-184)
185. [src/strategy_kochi_ppo.cpp](#file-185)
186. [src/strategy_market_making.cpp](#file-186)
187. [src/strategy_momentum_volume.cpp](#file-187)
188. [src/strategy_opening_range_breakout.cpp](#file-188)
189. [src/strategy_order_flow_imbalance.cpp](#file-189)
190. [src/strategy_order_flow_scalping.cpp](#file-190)
191. [src/strategy_signal_or.cpp](#file-191)
192. [src/strategy_sma_cross.cpp](#file-192)
193. [src/strategy_tfa.cpp](#file-193)
194. [src/strategy_vwap_reversion.cpp](#file-194)
195. [src/telemetry_logger.cpp](#file-195)
196. [src/temporal_analysis.cpp](#file-196)
197. [src/time_utils.cpp](#file-197)
198. [src/virtual_market.cpp](#file-198)

---

## 📄 **FILE 1 of 198**: audit/include/audit/audit_cli.hpp

**File Information**:
- **Path**: `audit/include/audit/audit_cli.hpp`

- **Size**: 15 lines
- **Modified**: 2025-09-11 19:50:36
...
namespace audit {
    ...
    void show_trade_flow(const std::string& db_path, const std::string& run_id, const std::string& symbol_filter = "", int limit = 10, bool enhanced = false);
    void show_signal_stats(const std::string& db_path, const std::string& run_id, const std::string& strategy_filter = "");
    void show_signal_flow(const std::string& db_path, const std::string& run_id, const std::string& symbol_filter = "", int limit = 10);
    void show_position_history(const std::string& db_path, const std::string& run_id, const std::string& symbol_filter = "", int limit = 20);
}

```

## 📄 **FILE 2 of 198**: audit/include/audit/audit_db.hpp

**File Information**:
- **Path**: `audit/include/audit/audit_db.hpp`

- **Size**: 101 lines
- **Modified**: 2025-09-12 02:37:41
...
## 📄 **FILE 3 of 198**: audit/include/audit/audit_db_recorder.hpp
...
public:
    explicit AuditDBRecorder(const std::string& db_path, const std::string& run_id);
    ~AuditDBRecorder();
...
## 📄 **FILE 4 of 198**: audit/include/audit/clock.hpp
...
## 📄 **FILE 5 of 198**: audit/include/audit/hash.hpp
...
## 📄 **FILE 6 of 198**: audit/include/audit/price_csv.hpp
...
## 📄 **FILE 7 of 198**: audit/src/audit_cli.cpp

**File Information**:
- **Path**: `audit/src/audit_cli.cpp`

- **Size**: 1300 lines
- **Modified**: 2025-09-12 22:56:21
...
static const char* usage =
  ...
  "  new-run [--db DB] --run RUN --strategy NAME --kind K --params FILE.json --data-hash HEX --git REV [--note TXT]\n"
  "  log [--db DB] --run RUN --ts MS --kind K [--symbol S] [--side SIDE] [--qty Q] [--price P] [--pnl D] [--weight W] [--prob P] [--reason R] [--note TXT]\n"
  "  end-run [--db DB] --run RUN\n"
  "  verify [--db DB] [--run RUN]     # defaults to latest run\n"
  "  summarize [--db DB] [--run RUN]   # defaults to latest run\n"
  "  export [--db DB] [--run RUN] --fmt jsonl|csv --out FILE  # defaults to latest run\n"
  "  grep [--db DB] [--run RUN] --sql \"WHERE ...\"  # defaults to latest run\n"
  "  diff [--db DB] --run A --run2 B\n"
  "  vacuum [--db DB]\n"
  "  list [--db DB] [--strategy NAME] [--kind K]\n"
  "  latest [--db DB] [--strategy NAME]\n"
  "  info [--db DB] [--run RUN]       # defaults to latest run\n"
  "  trade-flow [--db DB] [--run RUN] [--symbol S] [--limit N] [--enhanced]  # defaults to latest run\n"
  "  signal-stats [--db DB] [--run RUN] [--strategy NAME]  # defaults to latest run\n"
  "  signal-flow [--db DB] [--run RUN] [--symbol S] [--limit N]  # enhanced signal pipeline analysis\n"
  "  position-history [--db DB] [--run RUN] [--symbol S] [--limit N]  # position transitions with cash/equity\n"
  "\n"
  "Default database: audit/sentio_audit.sqlite3\n";
...
static std::string get_latest_run_id(const std::string& db_path) {
  try {
    DB db(db_path);
    // **FIX**: Order by run creation time (embedded in run_id), not data start time
    // started_at represents data start time, not run creation time
    // Extract timestamp from run_id for true chronological ordering
    std::string sql = "SELECT run_id FROM audit_runs ORDER BY CAST(SUBSTR(run_id, LENGTH(run_id) - 9) AS INTEGER) DESC LIMIT 1";
    ...
    if (sqlite3_step(st) == SQLITE_ROW) {
      ...
      std::string result = run_id ? run_id : "";
      sqlite3_finalize(st);
      return result;
    }
...
int audit_main(int argc, char** argv) {
  ...
  if (!strcmp(cmd,"trade-flow")) {
    ...
    const char* limit_str = arg("--limit", argc, argv, "10");
    int limit = std::atoi(limit_str);
    bool enhanced = has("--enhanced", argc, argv);
    show_trade_flow(dbp, run, symbol, limit, enhanced);
    return 0;
  }
  ...
  if (!strcmp(cmd,"signal-flow")) {
    ...
    const char* limit_str = arg("--limit", argc, argv, "10");
    int limit = std::atoi(limit_str);
    show_signal_flow(dbp, run, symbol, limit);
    return 0;
  }
  ...
  if (!strcmp(cmd,"position-history")) {
    ...
    int limit = std::atoi(limit_str);
    show_position_history(dbp, run, symbol, limit);
    return 0;
  }
...
void list_runs(const std::string& db_path, const std::string& strategy_filter, const std::string& kind_filter) {
  try {
    ...
    while (sqlite3_step(st) == SQLITE_ROW) {
      ...
      printf("%-30s %-15s %-10s %-20lld %-20lld %s\n", 
             run_id ? run_id : "", 
             strategy ? strategy : "", 
             kind ? kind : "", 
             started_at, 
...
void show_trade_flow(const std::string& db_path, const std::string& run_id, const std::string& symbol_filter, int limit, bool enhanced) {
  try {
    ...
    std::string sql = "SELECT ts_millis, kind, symbol, side, qty, price, pnl_delta, weight, prob, reason, note, hash_curr FROM audit_events WHERE run_id = ? AND kind IN ('SIGNAL', 'ORDER', 'FILL', 'SNAPSHOT')";
    ...
    printf("=== TRADE FLOW ANALYSIS ===\n");
    printf("Run ID: %s\n", run_id.c_str());
    if (!symbol_filter.empty()) {
      printf("Symbol Filter: %s\n", symbol_filter.c_str());
    }
    printf("Limit: %d events\n", limit);
    if (enhanced) {
      printf("Mode: Enhanced (with signal diagnostics)\n");
    }
    printf("\n");
    
    printf("┌─────────────────────────────────────────────────────────────────────────────────────────────┐\n");
    printf("│                                    TRADE FLOW DIAGRAM                                        │\n");
    printf("└─────────────────────────────────────────────────────────────────────────────────────────────┘\n\n");
    
    // Show signal diagnostics summary if enhanced mode
    if (enhanced) {
      printf("📊 SIGNAL DIAGNOSTICS SUMMARY:\n");
      printf("┌─────────────────────────────────────────────────────────────────────────────────────────────┐\n");
      
      // Get signal diagnostics for this run
      ...
      if (diag_rc == SQLITE_OK) {
        ...
        if (sqlite3_step(diag_st) == SQLITE_ROW) {
          ...
          printf("│ Strategy: %-20s │ Emitted: %-8.0f │ Dropped: %-8.0f │ Success: %-6.1f%% │\n", 
                 strategy ? strategy : "Unknown", emitted, dropped, 
    ...
    while (sqlite3_step(st) == SQLITE_ROW) {
      ...
      // **CLEAN DISPLAY**: Chain ID kept internally but not displayed to user
      (void)chain_id; // Suppress unused variable warning
      ...
      printf("│ %-8s │ %-8s │ %-6s │ %-8s │ %-10s │ %-8s │ %-12s │ %-8s │ %-8s │\n",
             time_str, kind ? kind : "", symbol ? symbol : "", 
             side ? side : "", 
             qty > 0 ? std::to_string(qty).c_str() : "",
             price > 0 ? std::to_string(price).c_str() : "",
             pnl_delta != 0 ? std::to_string(pnl_delta).c_str() : "",
             weight != 0 ? std::to_string(weight).c_str() : "",
             prob != 0 ? std::to_string(prob).c_str() : "");
      
      // Add detailed information based on event type
      if (kind) {
        if (strcmp(kind, "SIGNAL") == 0) {
          printf("│   📡 SIGNAL: %s %s (Prob: %.3f, Weight: %.3f)\n", 
                 symbol ? symbol : "", side ? side : "", prob, weight);
          if (reason) printf("│   Reason: %s\n", reason);
        } else if (strcmp(kind, "ORDER") == 0) {
          printf("│   📋 ORDER: %s %s %.0f @ %.2f\n", 
                 side ? side : "", symbol ? symbol : "", qty, price);
        } else if (strcmp(kind, "FILL") == 0) {
          printf("│   ✅ FILL: %s %.0f @ %.2f (P&L: %.2f)\n", 
                 symbol ? symbol : "", qty, price, pnl_delta);
          printf("│   💰 Trade completed with P&L impact\n");
        } else if (strcmp(kind, "SNAPSHOT") == 0) {
          printf("│   📊 SNAPSHOT: Portfolio state update\n");
        }
      }
    ...
    // **CLEAN DISPLAY**: Events displayed in simple chronological order
    
    // **CONFLICT DETECTION**: Analyze position conflicts in trade flow
    printf("\n🔍 POSITION CONFLICT ANALYSIS:\n");
    printf("┌─────────────────────────────────────────────────────────────────────────────────────────────┐\n");
    
    // Track positions by replaying fills in this trade flow
    std::unordered_map<std::string, ConflictPosition> flow_positions;
    int flow_conflicts = 0;
    
    // Re-query FILL events for conflict analysis
    sqlite3_stmt* conflict_st = nullptr;
    std::string conflict_sql = "SELECT ts_millis, symbol, side, qty FROM audit_events WHERE run_id = ? AND kind = 'FILL'";
    if (!symbol_filter.empty()) {
        conflict_sql += " AND symbol = '" + symbol_filter + "'";
    }
    conflict_sql += " ORDER BY ts_millis ASC";
    if (limit > 0) {
        conflict_sql += " LIMIT " + std::to_string(limit);
    }
    
    int conflict_rc = sqlite3_prepare_v2(db.get_db(), conflict_sql.c_str(), -1, &conflict_st, nullptr);
    if (conflict_rc == SQLITE_OK) {
        sqlite3_bind_text(conflict_st, 1, run_id.c_str(), -1, SQLITE_TRANSIENT);
        
        while (sqlite3_step(conflict_st) == SQLITE_ROW) {
            int64_t ts_millis = sqlite3_column_int64(conflict_st, 0);
            const char* symbol = (const char*)sqlite3_column_text(conflict_st, 1);
            const char* side = (const char*)sqlite3_column_text(conflict_st, 2);
            double qty = sqlite3_column_double(conflict_st, 3);
            
            if (symbol && side) {
                auto& pos = flow_positions[symbol];
                pos.symbol = symbol;
                ...
                auto conflict_analysis = analyze_position_conflicts(flow_positions);
                if (conflict_analysis.has_conflicts) {
                    flow_conflicts++;
                }
            }
        }
        sqlite3_finalize(conflict_st);
    }
    
    if (flow_conflicts == 0) {
        printf("│ ✅ NO CONFLICTS: All positions maintained proper directional consistency          │\n");
    } else {
        printf("│ ❌ CONFLICTS DETECTED: %d instances of conflicting positions in this flow        │\n", flow_conflicts);
        printf("│    ⚠️  These conflicts generate fees without profit and cause ETF decay         │\n");
    }
    
    printf("└─────────────────────────────────────────────────────────────────────────────────────────────┘\n");
    
    printf("\n📊 LEGEND:\n");
    printf("  📡 SIGNAL  → Strategy generates trading signal\n");
    printf("  🛣️  ROUTER → Routes signal to appropriate instrument\n");
    printf("  📋 ORDER   → Order placed in market\n");
    printf("  ✅ FILL    → Order executed (trade completed)\n");
    printf("  📊 SNAPSHOT → Portfolio state update\n");
...
void show_signal_flow(const std::string& db_path, const std::string& run_id, const std::string& symbol_filter, int limit) {
  try {
    DB db(db_path);
    
    printf("=== SIGNAL PIPELINE ANALYSIS ===\n");
    printf("Run ID: %s\n", run_id.c_str());
    if (!symbol_filter.empty()) {
      printf("Symbol Filter: %s\n", symbol_filter.c_str());
    }
    ...
    while (sqlite3_step(st) == SQLITE_ROW) {
      ...
      printf("│ %-8s │ %-8s │ %-6s │ %-6s │ %-6.3f │ %-6.3f │ %-7s │\n",
             time_str, kind ? kind : "", symbol ? symbol : "", 
             side ? side : "", prob, weight, status.c_str());
      
      // Add detailed information
      if (kind && strcmp(kind, "SIGNAL_DROP") == 0) {
        printf("│         │ └─ Drop Reason: %-60s │\n", reason ? reason : "");
      } else if (kind && strcmp(kind, "FILL") == 0) {
        printf("│         │ └─ P&L Impact: $%-10.2f │\n", pnl_delta);
      }
    }
...
void show_position_history(const std::string& db_path, const std::string& run_id, const std::string& symbol_filter, int limit) {
  try {
    DB db(db_path);
    
    printf("=== POSITION TRANSITION HISTORY ===\n");
    printf("Run ID: %s\n", run_id.c_str());
    if (!symbol_filter.empty()) {
      printf("Symbol Filter: %s\n", symbol_filter.c_str());
    }
    ...
    // Show position management explanation
    printf("📊 POSITION MANAGEMENT SYSTEM:\n");
    printf("┌─────────────────────────────────────────────────────────────────────────────────────────────┐\n");
    printf("│ Starting Cash: $100,000 → Position Changes → Cash Settlement → New Positions → Total Equity │\n");
    printf("└─────────────────────────────────────────────────────────────────────────────────────────────┘\n\n");
    
    // Get position-related events (FILL and SNAPSHOT events)
    std::string sql = "SELECT ts_millis, kind, symbol, side, qty, price, pnl_delta, note FROM audit_events WHERE run_id = ? AND kind IN ('FILL', 'SNAPSHOT')";
    ...
    printf("💰 POSITION TRANSITIONS:\n");
    printf("┌─────────────────────────────────────────────────────────────────────────────────────────────┐\n");
    printf("│ Time     │ Event    │ Symbol │ Side │ Qty      │ Price    │ Cash Δ    │ Position Value │ Equity │\n");
    printf("├─────────────────────────────────────────────────────────────────────────────────────────────┤\n");
    
    double running_cash = 100000.0; // Starting cash
    double running_equity = 100000.0; // Starting equity
    std::map<std::string, double> positions; // Track current positions
    std::map<std::string, double> avg_prices; // Track average prices
    ...
    while (sqlite3_step(st) == SQLITE_ROW) {
      ...
      double price = sqlite3_column_double(st, 5);
      double pnl_delta = sqlite3_column_double(st, 6);
      const char* note = reinterpret_cast<const char*>(sqlite3_column_text(st, 7));
      ...
      if (kind && strcmp(kind, "FILL") == 0) {
        // Calculate cash impact
        double cash_delta = (side && strcmp(side, "BUY") == 0) ? -(qty * price) : (qty * price);
        running_cash += cash_delta;
        
        // Update position and average price
        if (symbol) {
          double old_qty = positions[symbol];
          double new_qty = old_qty + ((side && strcmp(side, "BUY") == 0) ? qty : -qty);
          
          // Update average price using VWAP logic
          if (std::abs(new_qty) < 1e-6) {
            ...
            if (old_qty * new_qty >= 0) {
              ...
              if (std::abs(old_qty) > 1e-6) {
                avg_prices[symbol] = (avg_prices[symbol] * old_qty + price * qty) / new_qty;
              } else {
                avg_prices[symbol] = price;
              }
            } else {
              // Flipping direction - new average is fill price
              avg_prices[symbol] = price;
            }
        ...
        for (const auto& [sym, qty] : positions) {
          if (std::abs(qty) > 1e-6) {
            // Use current fill price for the symbol being traded, otherwise use average price as approximation
            double current_price = (sym == symbol) ? price : avg_prices[sym];
            total_position_value += qty * current_price;
          }
        }
        
        // Calculate equity = cash + position value
        running_equity = running_cash + total_position_value;
        
        printf("│ %-8s │ %-8s │ %-6s │ %-4s │ %-8.2f │ %-8.2f │ %-9.2f │ %-13.2f │ %-8.2f │\n",
               time_str, kind, symbol ? symbol : "", side ? side : "", qty, price, cash_delta, total_position_value, running_equity);
        
        // Show position change details
        if (symbol) {
          printf("│         │ └─ Position: %s %.2f shares @ $%.2f avg (Cash: $%.2f, Equity: $%.2f)\n", 
                 symbol, positions[symbol], avg_prices[symbol], running_cash, running_equity);
        }
        
      } else if (kind && strcmp(kind, "SNAPSHOT") == 0) {
        // Parse snapshot data
        double cash = 100000.0, equity = 100000.0;
        if (note) {
          std::string note_str = note;
          size_t cash_pos = note_str.find("cash=");
          size_t equity_pos = note_str.find("equity=");
          if (cash_pos != std::string::npos) {
            cash = std::stod(note_str.substr(cash_pos + 5));
          }
          if (equity_pos != std::string::npos) {
            equity = std::stod(note_str.substr(equity_pos + 7));
          }
        }
        ...
        // Calculate position value from snapshot
        double position_value = equity - cash;
        
        printf("│ %-8s │ %-8s │ %-6s │ %-4s │ %-8s │ %-8s │ %-9s │ %-13.2f │ %-8.2f │\n",
               time_str, kind, "ALL", "", "", "", "", position_value, running_equity);
        printf("│         │ └─ Portfolio Snapshot (Cash: $%.2f, Position Value: $%.2f, Equity: $%.2f)\n", cash, position_value, equity);
      }
    }
    ...
    printf("┌─────────────────────────────────────────────────────────────────────────────────────────────┐\n");
    printf("│ Symbol │ Position │ Avg Price │ Market Value │ Unrealized P&L │ Cash      │ Total Equity │\n");
    printf("├─────────────────────────────────────────────────────────────────────────────────────────────┤\n");
    ...
    for (const auto& [symbol, qty] : positions) {
      if (std::abs(qty) > 1e-6) {
        ...
        double current_price = 450.0; // Would need current market price for accurate calculation
        double market_value = qty * current_price;
        double unrealized_pnl = qty * (current_price - avg_price);
        total_position_value += market_value;
        total_unrealized_pnl += unrealized_pnl;
        
        printf("│ %-6s │ %-8.2f │ %-9.2f │ %-12.2f │ %-14.2f │ %-9.2f │ %-12.2f │\n",
               symbol.c_str(), qty, avg_price, market_value, unrealized_pnl, running_cash, running_equity);
      }
    }
    ...
    printf("  📊 Total Return: %.2f%%\n", ((running_equity - 100000.0) / 100000.0) * 100.0);
    
    printf("\n📋 TERMINOLOGY:\n");
    printf("  💰 Cash: Liquid money available for trading\n");
    printf("  📊 Position Value: Current market value of holdings (shares × current price)\n");
    printf("  📈 Unrealized P&L: Current profit/loss on open positions\n");
    printf("  🏦 Total Equity: Cash + Position Value (total account value)\n");
...
## 📄 **FILE 8 of 198**: audit/src/audit_db.cpp

**File Information**:
- **Path**: `audit/src/audit_db.cpp`

- **Size**: 344 lines
- **Modified**: 2025-09-12 23:56:53
...
std::pair<bool,std::string> DB::verify_run(const std::string& run_id) {
  // Check seq monotonic, recompute chain
  sqlite3_stmt* st=nullptr;
  ...
  while (sqlite3_step(st) == SQLITE_ROW) {
    auto seq = sqlite3_column_int64(st,0);
    if (seq != expected_seq) { sqlite3_finalize(st); return {false, "non-monotonic seq at " + std::to_string(seq)}; }
    Event ev;
    ev.run_id = run_id;
    ...
    if (hash_prev != prev) { sqlite3_finalize(st); return {false, "hash_prev mismatch at seq " + std::to_string(seq)}; }

    std::string content = canonical_content_string(seq, ev);
    std::string content_hash = sha256_hex(content);
    std::string recomputed = sha256_hex(prev + "\n" + content_hash);
    if (recomputed != hash_curr) { sqlite3_finalize(st); return {false, "hash_curr mismatch at seq " + std::to_string(seq)}; }
...
DB::Summary DB::summarize(const std::string& run_id) {
  ...
  if (s.ts_first > 0 && s.ts_last > 0 && s.n_fill > 0) {
    ...
    double time_span_days = (s.ts_last - s.ts_first) / (1000.0 * 60.0 * 60.0 * 24.0);
    ...
    // **FIX**: Calculate monthly projected return using proper compound interest
    // Convert total return percentage to decimal for compounding calculation
    double total_return_decimal = s.total_return / 100.0;
    double years = time_span_days / 365.25; // More accurate year calculation
    double annual_return = years > 0 ? std::pow(1.0 + total_return_decimal, 1.0/years) - 1.0 : 0.0;
    s.mpr = (std::pow(1.0 + annual_return, 1.0/12.0) - 1.0) * 100.0; // Convert back to percentage
...
## 📄 **FILE 9 of 198**: audit/src/audit_db_recorder.cpp
...
AuditDBRecorder::AuditDBRecorder(const std::string& db_path, const std::string& run_id)
    : run_id_(run_id), logging_enabled_(true), started_at_(0) {
...
void AuditDBRecorder::event_run_start(std::int64_t ts, const std::string& meta) {
    ...
    run.git_rev = "unknown";
    run.note = "Generated by sentio_cli";
...
## 📄 **FILE 10 of 198**: audit/src/clock.cpp
...
## 📄 **FILE 11 of 198**: audit/src/hash.cpp
...
## 📄 **FILE 12 of 198**: audit/src/price_csv.cpp
...
## 📄 **FILE 13 of 198**: include/sentio/all_strategies.hpp

**File Information**:
- **Path**: `include/sentio/all_strategies.hpp`

- **Size**: 18 lines
- **Modified**: 2025-09-12 11:16:22
...
## 📄 **FILE 14 of 198**: include/sentio/alpha.hpp
...
## 📄 **FILE 15 of 198**: include/sentio/alpha/sota_linear_policy.hpp
...
## 📄 **FILE 16 of 198**: include/sentio/audit.hpp
...
## 📄 **FILE 17 of 198**: include/sentio/audit_interface.hpp
...
## 📄 **FILE 18 of 198**: include/sentio/audit_validator.hpp
...
## 📄 **FILE 19 of 198**: include/sentio/base_strategy.hpp
...
## 📄 **FILE 20 of 198**: include/sentio/binio.hpp
...
## 📄 **FILE 21 of 198**: include/sentio/bo.hpp
...
## 📄 **FILE 22 of 198**: include/sentio/bollinger.hpp
...
## 📄 **FILE 23 of 198**: include/sentio/core.hpp

**File Information**:
- **Path**: `include/sentio/core.hpp`

- **Size**: 74 lines
- **Modified**: 2025-09-11 20:28:24
...
## 📄 **FILE 24 of 198**: include/sentio/core/bar.hpp
...
## 📄 **FILE 25 of 198**: include/sentio/cost_model.hpp
...
## 📄 **FILE 26 of 198**: include/sentio/csv_loader.hpp
...
## 📄 **FILE 27 of 198**: include/sentio/data_downloader.hpp
...
## 📄 **FILE 28 of 198**: include/sentio/data_resolver.hpp
...
## 📄 **FILE 29 of 198**: include/sentio/day_index.hpp
...
## 📄 **FILE 30 of 198**: include/sentio/exec/asof_index.hpp
...
## 📄 **FILE 31 of 198**: include/sentio/exec_types.hpp
...
## 📄 **FILE 32 of 198**: include/sentio/execution/pnl_engine.hpp
...
## 📄 **FILE 33 of 198**: include/sentio/family_mapper.hpp
...
## 📄 **FILE 34 of 198**: include/sentio/feature/column_projector.hpp
...
## 📄 **FILE 35 of 198**: include/sentio/feature/column_projector_safe.hpp
...
## 📄 **FILE 36 of 198**: include/sentio/feature/csv_feature_provider.hpp
...
## 📄 **FILE 37 of 198**: include/sentio/feature/feature_builder_guarded.hpp
...
## 📄 **FILE 38 of 198**: include/sentio/feature/feature_builder_ops.hpp
...
## 📄 **FILE 39 of 198**: include/sentio/feature/feature_feeder_guarded.hpp
...
## 📄 **FILE 40 of 198**: include/sentio/feature/feature_from_spec.hpp
...
## 📄 **FILE 41 of 198**: include/sentio/feature/feature_matrix.hpp
...
## 📄 **FILE 42 of 198**: include/sentio/feature/feature_provider.hpp
...
## 📄 **FILE 43 of 198**: include/sentio/feature/name_diff.hpp
...
## 📄 **FILE 44 of 198**: include/sentio/feature/ops.hpp
...
## 📄 **FILE 45 of 198**: include/sentio/feature/sanitize.hpp
...
## 📄 **FILE 46 of 198**: include/sentio/feature/standard_scaler.hpp
...
## 📄 **FILE 47 of 198**: include/sentio/feature_builder.hpp
...
## 📄 **FILE 48 of 198**: include/sentio/feature_cache.hpp
...
## 📄 **FILE 49 of 198**: include/sentio/feature_engineering/feature_normalizer.hpp
...
## 📄 **FILE 50 of 198**: include/sentio/feature_engineering/kochi_features.hpp
...
## 📄 **FILE 51 of 198**: include/sentio/feature_engineering/technical_indicators.hpp
...
## 📄 **FILE 52 of 198**: include/sentio/feature_feeder.hpp
...
## 📄 **FILE 53 of 198**: include/sentio/feature_health.hpp
...
## 📄 **FILE 54 of 198**: include/sentio/feature_utils.hpp
...
## 📄 **FILE 55 of 198**: include/sentio/indicators.hpp
...
## 📄 **FILE 56 of 198**: include/sentio/mars_data_loader.hpp
...
## 📄 **FILE 57 of 198**: include/sentio/metrics.hpp
...
## 📄 **FILE 58 of 198**: include/sentio/ml/feature_pipeline.hpp
...
## 📄 **FILE 59 of 198**: include/sentio/ml/feature_window.hpp
...
## 📄 **FILE 60 of 198**: include/sentio/ml/iml_model.hpp
...
## 📄 **FILE 61 of 198**: include/sentio/ml/model_registry.hpp
...
## 📄 **FILE 62 of 198**: include/sentio/ml/ts_model.hpp
...
## 📄 **FILE 63 of 198**: include/sentio/of_index.hpp
...
## 📄 **FILE 64 of 198**: include/sentio/of_precompute.hpp
...
## 📄 **FILE 65 of 198**: include/sentio/optimizer.hpp
...
## 📄 **FILE 66 of 198**: include/sentio/orderflow_types.hpp
...
## 📄 **FILE 67 of 198**: include/sentio/pnl_accounting.hpp
...
## 📄 **FILE 68 of 198**: include/sentio/polygon_client.hpp
...
## 📄 **FILE 69 of 198**: include/sentio/portfolio/alpaca_fee_model.hpp
...
## 📄 **FILE 70 of 198**: include/sentio/portfolio/capital_manager.hpp
...
## 📄 **FILE 71 of 198**: include/sentio/portfolio/fee_model.hpp
...
## 📄 **FILE 72 of 198**: include/sentio/portfolio/portfolio_allocator.hpp
...
## 📄 **FILE 73 of 198**: include/sentio/portfolio/tc_slippage_model.hpp
...
## 📄 **FILE 74 of 198**: include/sentio/portfolio/utilization_governor.hpp
...
## 📄 **FILE 75 of 198**: include/sentio/position_coordinator.hpp
...
## 📄 **FILE 76 of 198**: include/sentio/position_guardian.hpp
...
## 📄 **FILE 77 of 198**: include/sentio/position_manager.hpp
...
## 📄 **FILE 78 of 198**: include/sentio/position_orchestrator.hpp
...
## 📄 **FILE 79 of 198**: include/sentio/position_validator.hpp
...
## 📄 **FILE 80 of 198**: include/sentio/pricebook.hpp
...
## 📄 **FILE 81 of 198**: include/sentio/profiling.hpp
...
## 📄 **FILE 82 of 198**: include/sentio/progress_bar.hpp
...
## 📄 **FILE 83 of 198**: include/sentio/property_test.hpp
...
## 📄 **FILE 84 of 198**: include/sentio/rolling_stats.hpp
...
## 📄 **FILE 85 of 198**: include/sentio/router.hpp
...
## 📄 **FILE 86 of 198**: include/sentio/rsi_prob.hpp
...
## 📄 **FILE 87 of 198**: include/sentio/rsi_strategy.hpp
...
## 📄 **FILE 88 of 198**: include/sentio/rules/adapters.hpp
...
## 📄 **FILE 89 of 198**: include/sentio/rules/bbands_squeeze_rule.hpp
...
## 📄 **FILE 90 of 198**: include/sentio/rules/diversity_weighter.hpp
...
## 📄 **FILE 91 of 198**: include/sentio/rules/integrated_rule_ensemble.hpp
...
## 📄 **FILE 92 of 198**: include/sentio/rules/irule.hpp
...
## 📄 **FILE 93 of 198**: include/sentio/rules/momentum_volume_rule.hpp
...
## 📄 **FILE 94 of 198**: include/sentio/rules/ofi_proxy_rule.hpp
...
## 📄 **FILE 95 of 198**: include/sentio/rules/online_platt_calibrator.hpp
...
## 📄 **FILE 96 of 198**: include/sentio/rules/opening_range_breakout_rule.hpp
...
## 📄 **FILE 97 of 198**: include/sentio/rules/registry.hpp
...
## 📄 **FILE 98 of 198**: include/sentio/rules/sma_cross_rule.hpp
...
## 📄 **FILE 99 of 198**: include/sentio/rules/utils/validation.hpp
...
## 📄 **FILE 100 of 198**: include/sentio/rules/vwap_reversion_rule.hpp
...
## 📄 **FILE 101 of 198**: include/sentio/runner.hpp
...
## 📄 **FILE 102 of 198**: include/sentio/sanity.hpp
...
## 📄 **FILE 103 of 198**: include/sentio/side.hpp
...
## 📄 **FILE 104 of 198**: include/sentio/signal.hpp
...
## 📄 **FILE 105 of 198**: include/sentio/signal_diag.hpp
...
## 📄 **FILE 106 of 198**: include/sentio/signal_engine.hpp
...
## 📄 **FILE 107 of 198**: include/sentio/signal_gate.hpp
...
## 📄 **FILE 108 of 198**: include/sentio/signal_or.hpp
...
## 📄 **FILE 109 of 198**: include/sentio/signal_pipeline.hpp
...
## 📄 **FILE 110 of 198**: include/sentio/signal_trace.hpp
...
## 📄 **FILE 111 of 198**: include/sentio/signal_utils.hpp
...
## 📄 **FILE 112 of 198**: include/sentio/sim_data.hpp
...
## 📄 **FILE 113 of 198**: include/sentio/sizer.hpp
...
## 📄 **FILE 114 of 198**: include/sentio/strategy/intraday_position_governor.hpp
...
## 📄 **FILE 115 of 198**: include/sentio/strategy_bollinger_squeeze_breakout.hpp
...
## 📄 **FILE 116 of 198**: include/sentio/strategy_ire.hpp
...
## 📄 **FILE 117 of 198**: include/sentio/strategy_kochi_ppo.hpp
...
## 📄 **FILE 118 of 198**: include/sentio/strategy_market_making.hpp
...
## 📄 **FILE 119 of 198**: include/sentio/strategy_momentum_volume.hpp
...
## 📄 **FILE 120 of 198**: include/sentio/strategy_opening_range_breakout.hpp
...
## 📄 **FILE 121 of 198**: include/sentio/strategy_order_flow_imbalance.hpp
...
## 📄 **FILE 122 of 198**: include/sentio/strategy_order_flow_scalping.hpp
...
## 📄 **FILE 123 of 198**: include/sentio/strategy_signal_or.hpp
...
## 📄 **FILE 124 of 198**: include/sentio/strategy_sma_cross.hpp
...
## 📄 **FILE 125 of 198**: include/sentio/strategy_tfa.hpp
...
## 📄 **FILE 126 of 198**: include/sentio/strategy_transformer.hpp
...
## 📄 **FILE 127 of 198**: include/sentio/strategy_utils.hpp
...
## 📄 **FILE 128 of 198**: include/sentio/strategy_vwap_reversion.hpp
...
## 📄 **FILE 129 of 198**: include/sentio/sym/leverage_registry.hpp
...
## 📄 **FILE 130 of 198**: include/sentio/sym/symbol_utils.hpp
...
## 📄 **FILE 131 of 198**: include/sentio/symbol_table.hpp
...
## 📄 **FILE 132 of 198**: include/sentio/telemetry_logger.hpp
...
## 📄 **FILE 133 of 198**: include/sentio/temporal_analysis.hpp
...
## 📄 **FILE 134 of 198**: include/sentio/tfa/artifacts_loader.hpp
...
## 📄 **FILE 135 of 198**: include/sentio/tfa/artifacts_safe.hpp
...
## 📄 **FILE 136 of 198**: include/sentio/tfa/feature_guard.hpp
...
## 📄 **FILE 137 of 198**: include/sentio/tfa/input_shim.hpp
...
## 📄 **FILE 138 of 198**: include/sentio/tfa/signal_pipeline.hpp
...
## 📄 **FILE 139 of 198**: include/sentio/tfa/tfa_seq_context.hpp
...
## 📄 **FILE 140 of 198**: include/sentio/time_utils.hpp
...
## 📄 **FILE 141 of 198**: include/sentio/torch/safe_from_blob.hpp
...
## 📄 **FILE 142 of 198**: include/sentio/util/bytes.hpp
...
## 📄 **FILE 143 of 198**: include/sentio/util/safe_matrix.hpp
...
## 📄 **FILE 144 of 198**: include/sentio/utils/formatting.hpp
...
## 📄 **FILE 145 of 198**: include/sentio/utils/validation.hpp
...
## 📄 **FILE 146 of 198**: include/sentio/virtual_market.hpp
...
## 📄 **FILE 147 of 198**: include/sentio/wf.hpp
...
## 📄 **FILE 149 of 198**: src/audit_validator.cpp
...
AuditValidator::ValidationResult AuditValidator::validate_strategy_audit_compatibility(
    ...
    try {
        ...
        std::string run_id = "audit_validation_" + strategy_name + "_" + 
...
## 📄 **FILE 150 of 198**: src/base_strategy.cpp
...
## 📄 **FILE 151 of 198**: src/csv_loader.cpp

**File Information**:
- **Path**: `src/csv_loader.cpp`

- **Size**: 129 lines
- **Modified**: 2025-09-12 19:28:04
...
## 📄 **FILE 152 of 198**: src/data_downloader.cpp
...
## 📄 **FILE 153 of 198**: src/feature_builder.cpp
...
## 📄 **FILE 154 of 198**: src/feature_cache.cpp
...
## 📄 **FILE 155 of 198**: src/feature_engineering/feature_normalizer.cpp
...
## 📄 **FILE 156 of 198**: src/feature_engineering/kochi_features.cpp
...
## 📄 **FILE 157 of 198**: src/feature_engineering/technical_indicators.cpp
...
## 📄 **FILE 158 of 198**: src/feature_feeder.cpp
...
## 📄 **FILE 159 of 198**: src/feature_feeder_guarded.cpp
...
## 📄 **FILE 160 of 198**: src/feature_health.cpp
...
void usage() {
    std::cout << "Usage: sentio_cli <command> [options]\n"
              << "Commands:\n"
              << "  probe                    Show data availability, strategies, and system status\n"
              << "  audit-validate           Validate that all strategies work with the audit system\n"
              << "  download <symbol> [--years <n>] [--months <n>] [--days <n>] [--timespan day|hour|minute] [--no-holidays]\n"
              << "  tpatest <symbol> [--strategy <name>] [--strategy-all] [--params <k=v,...>] [--quarters <n>] [--weeks <n>] [--days <n>]\n"
              ...
              << "\nDownload Options (defaults: --years 3 --timespan minute --no-holidays):\n"
              << "  --years <n>       Download n years of data (default: 3)\n"
              << "  --months <n>      Download n months of data\n"
              << "  --days <n>        Download n days of data\n"
              << "  --timespan <t>    Data resolution: day, hour, minute (default: minute)\n"
              << "  --no-holidays     Exclude US market holidays - adds _NH suffix (default)\n"
              << "  --holidays        Include US market holidays\n"
...
## 📄 **FILE 162 of 198**: src/mars_data_loader.cpp
...
## 📄 **FILE 163 of 198**: src/ml/model_registry_ts.cpp
...
## 📄 **FILE 164 of 198**: src/ml/ts_model.cpp
...
## 📄 **FILE 165 of 198**: src/optimizer.cpp
...
## 📄 **FILE 166 of 198**: src/pnl_accounting.cpp
...
## 📄 **FILE 167 of 198**: src/poly_fetch_main.cpp
...
## 📄 **FILE 168 of 198**: src/polygon_client.cpp
...
## 📄 **FILE 169 of 198**: src/position_coordinator.cpp
...
## 📄 **FILE 170 of 198**: src/position_guardian.cpp
...
## 📄 **FILE 171 of 198**: src/position_orchestrator.cpp
...
## 📄 **FILE 172 of 198**: src/router.cpp
...
## 📄 **FILE 173 of 198**: src/rsi_strategy.cpp
...
## 📄 **FILE 174 of 198**: src/runner.cpp
...
## 📄 **FILE 175 of 198**: src/sanity.cpp
...
## 📄 **FILE 176 of 198**: src/signal_engine.cpp
...
## 📄 **FILE 177 of 198**: src/signal_gate.cpp
...
## 📄 **FILE 178 of 198**: src/signal_pipeline.cpp
...
## 📄 **FILE 179 of 198**: src/signal_trace.cpp
...
## 📄 **FILE 180 of 198**: src/sim_data.cpp
...
## 📄 **FILE 181 of 198**: src/strategy/run_rule_ensemble.cpp
...
## 📄 **FILE 182 of 198**: src/strategy_bollinger_squeeze_breakout.cpp
...
## 📄 **FILE 183 of 198**: src/strategy_initialization.cpp
...
## 📄 **FILE 184 of 198**: src/strategy_ire.cpp
...
## 📄 **FILE 185 of 198**: src/strategy_kochi_ppo.cpp
...
## 📄 **FILE 186 of 198**: src/strategy_market_making.cpp
...
## 📄 **FILE 187 of 198**: src/strategy_momentum_volume.cpp
...
## 📄 **FILE 188 of 198**: src/strategy_opening_range_breakout.cpp
...
## 📄 **FILE 189 of 198**: src/strategy_order_flow_imbalance.cpp
...
## 📄 **FILE 190 of 198**: src/strategy_order_flow_scalping.cpp
...
## 📄 **FILE 191 of 198**: src/strategy_signal_or.cpp
...
## 📄 **FILE 192 of 198**: src/strategy_sma_cross.cpp
...
## 📄 **FILE 193 of 198**: src/strategy_tfa.cpp
...
## 📄 **FILE 194 of 198**: src/strategy_vwap_reversion.cpp
...
## 📄 **FILE 195 of 198**: src/telemetry_logger.cpp
...
## 📄 **FILE 196 of 198**: src/temporal_analysis.cpp
...
## 📄 **FILE 197 of 198**: src/time_utils.cpp
...
## 📄 **FILE 198 of 198**: src/virtual_market.cpp
