# Zero Orders/Fills Problem - Complete Analysis

**Generated**: 2025-09-05 11:32:41
**Source Directory**: /Users/yeogirlyun/C++/sentio_cpp
**Description**: Comprehensive analysis of zero orders/fills problem affecting 75% of trading strategies
...
#include <algorithm>
#include <cmath>
#include <iostream> // For WARN messages

namespace sentio {

// **MODIFIED**: Constructor is simplified. The member-initializer list for rolling
// objects is removed as apply_params() is the single source of truth for their creation.
MarketMakingStrategy::MarketMakingStrategy() 
    : BaseStrategy("MarketMaking"),
      rolling_returns_(20),
      rolling_volume_(50) {
    params_ = get_default_params();
    apply_params(); // This handles all initialization based on parameters.
}
...
// **MODIFIED**: This function now defensively reads and validates window sizes
// before creating the rolling indicator objects. This prevents a crash even if
// invalid parameters (like 0) are provided.
void MarketMakingStrategy::apply_params() {
    ...
    // Defensively read and validate window sizes
    int vol_window = 20; // A safe default
    if (params_.count("volatility_window")) {
        int parsed_window = static_cast<int>(params_.at("volatility_window"));
        if (parsed_window > 0) {
            vol_window = parsed_window;
        } else {
            std::cerr << "WARN: Invalid volatility_window (" << parsed_window << "), using default " << vol_window << std::endl;
        }
    }

    int vol_mean_window = 50; // A safe default
    if (params_.count("volume_window")) {
        int parsed_window = static_cast<int>(params_.at("volume_window"));
        if (parsed_window > 0) {
            vol_mean_window = parsed_window;
        } else {
            std::cerr << "WARN: Invalid volume_window (" << parsed_window << "), using default " << vol_mean_window << std::endl;
        }
    }
    
    // Recreate rolling objects with validated window sizes
...
void MarketMakingStrategy::reset_state() {
    ...
    // Re-initialize rolling objects to clear their state (e.g., buffer, sum)
    if (params_.count("volatility_window")) {
        rolling_returns_ = RollingMeanVar(static_cast<int>(params_.at("volatility_window")));
    }
    if (params_.count("volume_window")) {
        rolling_volume_ = RollingMean(static_cast<int>(params_.at("volume_window")));
    }
}
